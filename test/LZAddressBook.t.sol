...
contract LZAddressBookTest is Test {
    ...
    function test_protocolProvider_getByChainName() public view {
        ILZProtocol.ProtocolAddresses memory addresses = protocolProvider.getProtocolAddresses("optimism-mainnet");
        ...
        console.log("  EID from chain ID:", protocolProvider.getEidFromChainId(uint256(10)));
    ...
    function test_protocolProvider_eidConversion() public view {
        // Test various chains
        assertEq(protocolProvider.getEidFromChainId(uint256(1)), 30101, "Ethereum EID");
        assertEq(protocolProvider.getEidFromChainId(uint256(42161)), 30110, "Arbitrum EID");
        assertEq(protocolProvider.getEidFromChainId(uint256(8453)), 30184, "Base EID");
        assertEq(protocolProvider.getEidFromChainId(uint256(10)), 30111, "Optimism EID");
        assertEq(protocolProvider.getEidFromChainId(uint256(137)), 30109, "Polygon EID");
    }
    ...
    function test_protocolProvider_chainSupport() public view {
        ...
        assertTrue(protocolProvider.isChainSupported(30101), "Ethereum should be supported");
        assertTrue(protocolProvider.isChainSupported(30110), "Arbitrum should be supported");
        assertTrue(protocolProvider.isChainSupported(30184), "Base should be supported");
        
        assertTrue(protocolProvider.isSupportedChainId(uint256(1)), "Ethereum chain ID should be supported");
        assertTrue(protocolProvider.isSupportedChainId(uint256(42161)), "Arbitrum chain ID should be supported");
        assertTrue(protocolProvider.isSupportedChainId(uint256(8453)), "Base chain ID should be supported");
        
        // This should not be supported (random EID)
        assertFalse(protocolProvider.isChainSupported(99999), "Random EID should not be supported");
    }
    ...
    function test_workersRegistry_getMultipleDVNs() public view {
        ...
        address[] memory dvnAddresses = workersRegistry.getDVNAddresses("base-mainnet", dvnNames);
    ...
    function test_workersRegistry_getAllDVNsForChain() public view {
        (string[] memory names, address[] memory addresses) = workersRegistry.getDVNsForChainByName("ethereum-mainnet");
